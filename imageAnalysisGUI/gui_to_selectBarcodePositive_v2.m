function S = gui_to_selectBarcodePositive_v2(S, tileSize,channelCy, channelDAPI)

    % set default image contrast to use:
    threshold_min = 1200/65535;
    threshold_max = 2800/65535;
    ...
    if ~isfield(S.nuclei,'barcodeFISH')
         S.nuclei.barcodeFISH = false(1,numel(S.nuclei.coords));
    end
...
%         bottomY = top_coords(frame) + tileSize(2);
    ...
    %         bottomY = top_coords(frame) + tileSize(2);
            ...
            % plot coordinates on image:
            scatter(inFrameNucleiCoords(:,1), inFrameNucleiCoords(:,2), 20, 'red', 'filled');      
    ...
    function callback_add(~, ~)
        ...
        %Set labels true
        temp_labels = S.nuclei.barcodeFISH;
        
        temp_labels(I) = true;
        
        S.nuclei.barcodeFISH = temp_labels;
        % view image:
        view_image;
