function S = gui_to_selectBarcodePositive(scanFile, S, channelCy, channelDapi)
    ...
    if ~isfield(S(position),'barcodeFISH')
         [S(1:imageCountN).barcodeFISH] = deal(struct('coords', []));
    end
...
%             temp_coords = temp_coords';
    ...
    function callback_add(~, ~)
        ...
        S(position).barcodeFISH.coords = temp_coords;
    ...
    function callback_clear(~, ~)
        ...
        if ~isempty([S(position).barcodeFISH.coords])
            ...
            [S(position).barcodeFISH] = deal(struct('coords', []));
