function S = gui_to_checkSensitivity(scanFile, S, channelCy, channelDapi)
    ...
    for i = 1:numel(GFPpositions)
        if ~isfield(S(GFPpositions(i)).GFP,'label')
            S(GFPpositions(i)).GFP.label = true(1,numel(S(GFPpositions(i)).GFP.coords));
        end
    end
    frame = 1; %Change to frames with GFP+ points
...
%     end
    ...
    function handles = create_GUI
        ...
        uicontrol('Style', 'text', ...
            ...
            'FontSize', 26, ...
            'String', 'Click add to select false negative cells: GFP+ (green or red spot), barcodeFISH-(no yellow circle). Be sure to press Enter before moving to Next frame. Green spot indicate true positive, red indicates false negative');
...
% callback tomove to next image in scan:
  ...
  % function to plot the points:
    function plot_FISH
        ...
        if ~isempty([S(GFPpositions(frame)).barcodeFISH.coords])
            % turn on the hold:
            hold(handles.image, 'on'); 
            
            % get coordinates:
            temp_coords = cell2mat(S(GFPpositions(frame)).barcodeFISH.coords);
...
%             temp_coords = temp_coords';
            ...
            scatter(temp_coords(:,1), temp_coords(:,2), 80, 'yellow');      
    ...
    function plot_GFP
        ...
        % get coordinates:
        temp_coords = cell2mat(S(GFPpositions(frame)).GFP.coords);
%         temp_coords = [S(GFPpositions(frame)).GFP.coords];
%         temp_coords = reshape(temp_coords, 2, []);
%         temp_coords = temp_coords';
        
        temp_labels = S(GFPpositions(frame)).GFP.label;
        
        % plot true positives green:
        scatter(temp_coords(temp_labels,1), temp_coords(temp_labels,2), 36, 'filled', 'green');  

        % plot false negative red:
        scatter(temp_coords(~temp_labels,1), temp_coords(~temp_labels,2), 36, 'filled', 'red');  
...
%         temp_coords = temp_coords';
        ...
        S(GFPpositions(frame)).GFP.label = temp_labels;
        % view image:
        view_image;
    ...
    function callback_clear(~, ~)
        ...
        S(GFPpositions(frame)).GFP.label = true(1,numel(S(GFPpositions(frame)).GFP.coords));
