function [] = parseTiles(scanFile, regions, wavelengths, varargin)
    ...
    p.addParameter('parse',true,@islogical);
    ...
    for i = 1:numel(regions.points)
        ...
        regions.stitch{i} = zeros(max(leftCoords)+dimensionY-1,max(topCoords)+dimensionX-1, numel(wavelengths), 'uint16');
    ...
    for i = 1:numel(regions.points)
        for ii = 1:numel(regions.tiles{i})
            for iii = 1:numel(wavelengths)
                ...
                regions.stitch{i}(leftCoords(ii):leftCoords(ii)+dimensionY-1, ...
        ...
        fprintf('Finished stitching wavelength %d for position %d\n',iii, i);
        end    
    end
    ...
    for i = 1:numel(regions.points)
        for ii = 1:numel(wavelengths)
            ...
            imwrite(imresize(regions.stitch{i}(:,:,ii), 0.5), fullfile(inDir, sprintf('stitch_point%d_w%d.tif', i, ii)))
            imwrite(scale(regions.stitch{i}(:,:,ii)), fullfile(inDir, sprintf('stitch_point%d_w%d.jpeg', i, ii)));
